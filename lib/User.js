...
class User extends BaseModel {
    ...
    static get(user, appId, callback) {
     	if (user.id) {
		    Services.datasource.dataStorage.getObjects([user.id], function(errs, results) {
			    ...
			    callback(null, new User(results[0].properties));
		    });
    ...
    static new(props, callback) {
        ...
        User.get({username: newUser.username}, newUser.application_id, (err, res) => {
            if (err && err.status == 404) {
			    Services.datasource.dataStorage.createObjects([newUser, userMetadata], (errs) => {
				    if (errs && errs.length) {
