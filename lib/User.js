...
User.delete = function(id, callback) {
	...
	async.waterfall([
		...
		function(results, callback1) {
			User.deleteUserRelatedKeys(id, callback1);
		}
	], callback);
}
...
User.deleteUserRelatedKeys = function(user, callback) {
	...
	async.waterfall([
				...
				async.each(user.subscriptions[deviceId], function(sub, c2) {
					...
					async.waterfall([
						function(callback2) {
							...
							Application.stateBucket.remove(deviceKey, callback2);
						},
						function(callback2) {
		...
		callback(null, deletedObjectKeys);
	});
};
