...
class Channel {
	...
	model(model, id) {
		if (model) {
			if (Application.isValid(id, model)) {
				this.props.model = model;
			} else if (Application.isBuiltIn(model))
				this.props.model = model;
			else
	...
	get(options) {
		...
		let operations = [[{
			...
			toReplace: '{parent_model}',
			props: this.props.parent.model
		}, {
			toReplace: '{parent_id}', 
			props: this.props.parent.id
		}], [{
			toReplace: '{id}', 
			props: this.props.modelId
		}]]; 
		...
		for (let bit = 0; bit <= 4; bit++) { 
			if ((1 << bit) & this.mask) {
				for(let r in operations[bit]) {
					validChannel = validChannel.replace(operations[bit][r].toReplace, operations[bit][r].props);
				}		
			}
		}
		switch(this.mask) {
