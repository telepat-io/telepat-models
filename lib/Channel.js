...
Channel.validChannels = {
	4:  Channel.keyPrefix+":{model}",											//channel used for built-in models (users, contexts, applications)
	5:  Channel.keyPrefix+":{context}:{model}",									//the Channel of all objects from a context
	7:  Channel.keyPrefix+":{context}:users:{user_id}:{model}",					//the Channel of all objects from a context from an user
	12: Channel.keyPrefix+":{parent_model}:{parent_id}:{model}",				//the Channel of all objects belong to a parent
	14: Channel.keyPrefix+":users:{user_id}:{parent_model}:{parent_id}:{model}",//the Channel of all comments from event 1 from user 16
	20: Channel.keyPrefix+":{model}:{id}",										//the Channel of one item
};

Channel.builtInModels = ['user', 'contexts', 'applications'];

Channel.prototype.model = function(model, id) {
	if (model) {
		if (Application.loadedAppModels[this.appId]!==undefined && Application.loadedAppModels[this.appId][model]) {
			this.props.model = Application.loadedAppModels[this.appId][model].namespace;
...
Channel.prototype.parent = function(parent) {
	if (parent && parent.model && parent.id) {
		...
		this.props.parent = {model: Application.loadedAppModels[this.appId][parent.model].namespace, id: parent.id};
	} else
		this.forceInvalid = true;
...
Channel.prototype.isValid = function() {
	...
	return !this.forceInvalid && (new Boolean(mask));
};
