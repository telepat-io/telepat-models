...
class Channel {
	...
	model(model, id) {
		if (model) {
			if (Application.apps[this.applicaiton_id] !== undefined && Application.apps[this.appId].schema !== undefined && Application.apps[this.appId].schema[model]) {
	...
	isValid() {
		...
		var result = !this.forceInvalid && (mask !== undefined);
