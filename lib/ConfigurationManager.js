...
class ConfigurationManager {
	...
	load(callback) {
		async.series([
			function(callback1) {
				fs.readFile(this.specFile, {encoding: 'utf8'}, function(err, contents) {
					if (err) {
			...
			function(callback1) {
				fs.readFile(this.configFile, {encoding: 'utf8'}, function(err, contents) {
					if (err) {
						callback1(TelepatError(TelepatError.errors.ServerConfigurationFailure, [err.message]));
					} else {
	...
	*/
	verifyletiable(letiable, specletiable, rootName) {
		...
		return specletiable.root ? this._validate(specletiable.root, dot.pick(fullletName.slice(1), this.config), fullletName) : true;
	}
