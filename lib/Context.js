					...
					Application.bucket.getMulti(results, function(err, results1) {
						...
						if(!err) {
							for(var m in results1) {
								var contextId = m.split(':').slice(-1)[0];
								allContexts[contextId] = results1[m].value;
							}
						}
...
Context.update = function(id, patches, callback) {
	async.waterfall([
		function(callback1) {
			new Context(id, callback1);
		},
		function(result, callback1) {
			var context = result;

			for (var i in patches) {
				if (patches.hasOwnProperty(i) && Context._model.properties[patches[i].path]) {
					switch (patches[i].op) {
						case 'replace': {
							context[patches[i].path] = patches[i].value;
						...
						case 'append': {
							if (Context._model.properties[patches[i].path].type == 'array') {
								if (context[patches[i].path].indexOf(patches[i].value) === -1)
									context[patches[i].path].push(patches[i].value);
							}
							else
								context[patches[i].path] += patches[i].value;
			...
			Application.bucket.replace(key, context, callback);
		}
	], callback);
}
