					...
					Application.bucket.getMulti(results1, function(err, results2) {
						...
						if(!err) {
							for(var m in results2) {
								...
								allContexts[contextId] = results2[m].value;
							}
						}
...
Context.update = function(id, props, callback) {
	...
	Application.bucket.replace(key, props, callback);
}
...
Context.delete = function(id, callback) {
	...
	async.waterfall([
		...
		function(result, callback1) {
			var query = cb.ViewQuery.from('dev_state_document', 'by_context').custom({inclusive_end: true, key: '"'+id+'"'});
			Application.stateBucket.query(query, callback1);
		},
		function(results, callback1) {
			async.each(results, function(item, c) {
				Application.stateBucket.delete(item.id, c);
			}, callback1);
		}
	], callback);
}
