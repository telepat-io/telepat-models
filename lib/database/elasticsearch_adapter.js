...
let builtinModels = {
    ...
    'admin': require('../Admin'),
    'user_metadata': function() {return this['user'];}
};
...
class ElasticSearchDB extends Main_Database_Adapter{
    constructor(config) {
        ...
        this.connection.ping({
            ...
            if (err) {
                let d = new Date();
                Services.logger.error(`Failed connecting to Elasticsearch "${self.config.host}": ${err.message}. Retrying...`);
                setTimeout(() => {
                    retryConnection();
                }, 1000);
            } else {
        ...
        function getAndUpdate(objectIds, callback2) {
            ...
            async.series([
                ...
                function updateBulk(callback1) {
                    async.forEachOf(objectIds, (patches, id, c) => {
