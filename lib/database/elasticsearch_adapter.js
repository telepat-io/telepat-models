	...
	async.series([
		...
		function updateBulk(callback1) {
			objects.forEach(function(object) {
				if (builtinModels.indexOf(object.type) !== -1)
					builtinDetected = true;

				object = Delta.processObject([ids[object.id]], object);
				processedObjects.push(object);
				var script = 'def jsonSlurper = new groovy.json.JsonSlurper();'+
					'def parsed = jsonSlurper.parseText(\''+JSON.stringify(object)+'\');'+
					'ctx._source = parsed;';

				bulk.push({update: {_type: object.type, _id: object.id}});
			...
			self.connection.bulk({
				...
				if (res.errors)
					res.items.forEach(function(error) {
						totalErrors.push(new TelepatError(TelepatError.errors.ServerFailure, ['Failed to update '+error.update._type+' with ID '+error.update._id]));
		...
		if (err)
			...
			callback(totalErrors, processedObjects);
	});
};
