...
ElasticSearchDB.prototype.searchObjects = function(options, callback) {
	...
	if (options.fields) {
		...
		reqBody._source = {exclude: ['_metadata']};
		...
		async.series([
			...
			function updateBulk(callback1) {
				async.forEachOf(objectIds, function(patches, id, c) {
					var objectModel = patches[0].path.split('/')[0];

					var processedObject = Delta.processObject(patches, dbObjects[id]);

					if (processedObject === false) {
						delete dbObjects[id];
						return c();
					} else {
						dbObjects[id] = processedObject;
					}
