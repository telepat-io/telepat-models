...
var ElasticSearchDB = function(config) {
	...
	esConfig.log = this.config.log !== undefined ? this.config.log : undefined;
...
ElasticSearchDB.prototype.updateObjects = function(patches, callback) {
	...
	patches.forEach(function(patch) {
		if (!patch.path || typeof patch.path != 'string') {
			return totalErrors.push(new TelepatError(TelepatError.errors.InvalidPatch,
		...
		async.series([
			...
			function updateBulk(callback1) {
				...
				async.forEachOf(objectPatches, function(patches, id, c) {
					...
					var result = Delta.processObject(patches, dbObjects[id]);
...
ElasticSearchDB.prototype.deleteObjects = function(ids, callback) {
	...
	async.each(Object.keys(ids), function(id, c) {
		...
		self.connection.bulk({
			...
			async.each(results.items, function(result, c) {
