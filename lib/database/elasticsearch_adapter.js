...
var ElasticSearchDB = function(config) {
	...
	this.connection.ping({
		...
		if (err) {
			...
			if (self.onReadyCallback instanceof Function)
...
ElasticSearchDB.prototype.getObjects = function(ids, callback) {
	if (!Array.isArray(ids) || ids.length === 0)
		return callback([new TelepatError(TelepatError.errors.InvalidFieldValue,
			['ElasticSearchDB.getObjects: "ids" should be a non-empty array'])], [], []);
...
ElasticSearchDB.prototype.createObjects = function(objects, callback) {
	if (!Array.isArray(objects) || objects.length === 0)
		return callback([new TelepatError(TelepatError.errors.InvalidFieldValue,
			['ElasticSearchDB.createObjects: "objects" should be a non-empty array'])], [], []);
	...
	if (bulk.length != objects.length*2) {
		Application.logger.warning('ElasticSearchDB.createObjects: some objects were missing their ' +
			'"type" and "id" ('+(objects.length - bulk.length/2)+' failed)');
	}
...
ElasticSearchDB.prototype.updateObjects = function(patches, callback) {
	if (!Array.isArray(patches) || patches.length === 0) {
		return callback(new TelepatError(TelepatError.errors.InvalidFieldValue,
			['ElasticSearchDB.updateObjects: "patches" should be a non-empty array']));
	}
	...
	patches.forEach(function(patch) {
		if (!patch.path || typeof patch.path != 'string')
			return Application.logger.warning(new TelepatError(TelepatError.errors.InvalidPatch, ['path is missing or invalid']));
		if (patch.path.split('/').length != 3)
			return Application.logger.warning(new TelepatError(TelepatError.errors.InvalidPatch, ['the path is malformed']));
	...
	function getAndUpdate(objectPatches, callback2) {
		...
		var localIds = Object.keys(objectPatches);
		var bulk = [];
		...
		async.series([
			function getObjects(callback1) {
				self.getObjects(localIds, function(err, results, versions) {
					totalErrors = err;
			...
			function updateBulk(callback1) {
				if (!Object.keys(dbObjects).length)
					return callback1();
				async.forEachOf(objectPatches, function(patches, id, c) {
