...
ElasticSearchDB.prototype.modelDelete = function(modelName, appId, context, id, onlyChild, callback) {
	...
	async.series([
		...
		function(callback1) {
			...
			var children = Application.loadedAppModels[appId].schema[modelName].hasMany || [];

			if (Application.loadedAppModels[appId].schema[modelName].hasSome)
				children = children.concat(Application.loadedAppModels[appId].schema[modelName].hasSome) || [];
			...
			async.each(children, function(child, c) {
				...
				self.connection.search(searchParams, function(err, results) {
					...
					async.each(results.hits.hits, function(item, c2) {
						self.modelDelete(item._source.type, appId, context, item.id, false, c2);
					}, c);
				});
			}, callback1);
		}
...
ElasticSearchDB.prototype.modelCreate = function(modelName, appId, props, callback) {
	...
	for (var r in Application.loadedAppModels[appId].schema[modelName].belongsTo) {
		if (props[Application.loadedAppModels[appId].schema[modelName].belongsTo[r].parentModel + '_id']) {
			parent = {
				model: Application.loadedAppModels[appId].schema[modelName].belongsTo[r].parentModel,
				id: props[Application.loadedAppModels[appId].schema[modelName].belongsTo[r].parentModel + '_id']
			};
			relationType = Application.loadedAppModels[appId].schema[modelName].belongsTo[r].relationType;
		}
	}

	if (relationType == 'hasSome') {
		parentRelationKey = props[Application.loadedAppModels[appId].schema[parent.model].hasSome_property+'_index'];
	}
			...
			if (parent) {
				self.modelGet(parent.model, appId, props.context_id, parent.id, function(err, result) {
					if (err && err.status == 404) {
						...
						return callback1(err);
					} else if (result[Application.loadedAppModels[appId].schema[parent.model].hasSome_property] &&
						result[Application.loadedAppModels[appId].schema[parent.model].hasSome_property].length <= parentRelationKey) {

						callback1(new TelepatError(TelepatError.errors.InvalidObjectRelationKey,
							[
								parentRelationKey,
								result[Application.loadedAppModels[appId].schema[parent.model].hasSome_property].length-1
							]));
					} else
						callback1();
				});
...
ElasticSearchDB.prototype.userDelete = function(email, appId, callback) {
	...
	var relatedAndSingleModelsQuery = [];
	...
	async.series([
		...
		function(callback1) {
			async.each(Object.keys(appModels), function(modelName, c) {
				if (Array.isArray(appModels[modelName].belongsTo)) {
					if (!appModels[modelName].belongsTo.length) {
						relatedAndSingleModelsQuery.push({terms: {type: modelName}});

						return c();
					}
				}
				if ((appModels[modelName].hasMany && appModels[modelName].hasMany.length) ||
					(appModels[modelName].hasSome && appModels[modelName].hasSome.length))
					relatedAndSingleModelsQuery.push({terms: {type: modelName}});
				c();
			}, callback1);
		},
		function(callback1) {
			...
			if (relatedAndSingleModelsQuery.length == 0)
				return callback1();

			console.log(JSON.stringify({query: {
				bool: {
					must: {term: {user_id: user.id}},
					should: relatedAndSingleModelsQuery,
