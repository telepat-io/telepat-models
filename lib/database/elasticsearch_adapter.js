...
ElasticSearchDB.prototype.applicationGetAll = function(offset, limit, callback) {
	this.connection.search({
		index: this.config.index,
		size: limit || this.config.get_limit,
		from: offset || 0,
		type: 'application',
		fields: ['_source']
	}, function(error, response) {
...
ElasticSearchDB.prototype.applicationDeleteModelSchema = function(appId, modelName, callback) {
	...
	var modelsToDelete = [modelName];
	var appSchema = Application.loadedAppModels[appId].schema;

	var getSiblings = function(model) {
		if (appSchema && appSchema[model] && appSchema[model].hasMany) {
			modelsToDelete = modelsToDelete.concat(appSchema[model].hasMany);

			appSchema[model].hasMany.forEach(function(child) {
				getSiblings(child);
			});
		}
		if (appSchema && appSchema[model] && appSchema[model].hasSome) {
			modelsToDelete = modelsToDelete.concat(appSchema[model].hasSome);

			appSchema[model].hasSome.forEach(function(child) {
				getSiblings(child);
			});
		}
	};

	getSiblings(modelName);
	...
	async.series([
		function(callback1) {
			...
			modelsToDelete.forEach(function(model) {
				script += 'ctx._source.schema.remove("'+model+'");'
			});
...
ElasticSearchDB.prototype.contextGet = function(id, callback) {
	this.connection.get({
		...
		callback(null, result._source, result._version);
	});
};

ElasticSearchDB.prototype.contextGetAll = function(appId, offset, limit, callback) {
	var searchQuery = {
		index: this.config.index,
		type: 'context',
		size: limit || this.config.get_limit,
...
ElasticSearchDB.prototype.contextUpdate = function(id, patches, callback) {
	...
	var getAndUpdate = function(callback1) {
		savedCallback = callback1;
		self.contextGet(id, callback1);
	};

	async.waterfall([
		getAndUpdate,
		function(context, version, callback1) {
			updatedContext = Delta.processObject(patches, context);
			var script = 'def jsonSlurper = new groovy.json.JsonSlurper();'+
				'def parsed = jsonSlurper.parseText(\''+JSON.stringify(updatedContext).replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/'/g, "\\'")+'\');'+
				'ctx._source = parsed;';
			...
			self.connection.update({
				...
				if (response.status == 409)
					getAndUpdate(savedCallback);
				else
					callback1(err, response);
			});
		}
	], function(err) {
		if (err) return callback(err);
...
ElasticSearchDB.prototype.modelGet = function(name, appId, context_id, id, callback) {
	this.connection.get({
		...
		callback(null, response._source, response._version);
	});
};
					...
					async.each(results.hits.hits, function(item, c2) {
						self.modelDelete(item._source.type, appId, context, item._source.id, false, function(err2) {
							if (err2 && err2.status == '404') {
								Application.logger.warning('Model.delete trying to delete children fail: not found '+
								'('+item._source.type+':'+item._source.id+')');
							}
							c2();
						});
					}, c);
...
ElasticSearchDB.prototype.modelCountByChannel = function(channel, aggregation, callback) {
	var userQuery = {};
	var parentQuery = {};
	...
	if (aggregation) {
		...
		if (channel.props.context) {
			elasticSearchQuery.query.bool.must.push({term: {context_id: channel.props.context}});
		}
		...
		elasticSearchQuery.aggs = {aggregation: aggregation};
		...
		if (channel.filter){
			...
			elasticSearchQuery.query.filtered.filter.and.push({term: {application_id: channel.appId}});
		} else
			elasticSearchQuery.query.filtered.filter = {and: [{term: {application_id: channel.appId}}]};

		if (channel.props.context) {
			elasticSearchQuery.query.filtered.filter.and.push({term: {context_id: channel.props.context}});
		}

		if (channel.props.user) {
			userQuery['user_id'] = channel.props.user;
			elasticSearchQuery.query.filtered.filter.and.push({term: userQuery});
		}

		if(channel.props.parent) {
			parentQuery[channel.props.parent.model+'_id'] = channel.props.parent.id;
			elasticSearchQuery.query.filtered.filter.and.push({term: parentQuery});
		}
...
ElasticSearchDB.prototype.modelUpdate = function(modelName, appId, context, id, patch, callback) {
	...
	async.waterfall([
		getAndUpdate,
		function(object, version, callback1) {
			object = Delta.processObject(patch, object);
			var script = 'def jsonSlurper = new groovy.json.JsonSlurper();'+
				'def parsed = jsonSlurper.parseText(\''+JSON.stringify(object).replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/'/g, "\\'")+'\');'+
				'ctx._source = parsed;';
 ...
 * @param callback
 */
ElasticSearchDB.prototype.modelSearch = function(channel, sort, offset, limit, callback) {
	var userQuery = {};
	var parentQuery = {};
	var elasticSearchQuery = {
		query: {
	...
	this.connection.search({
		...
		body: elasticSearchQuery,
		from: offset || 0,
		size: limit || this.config.subscribe_limit
	}, function(err, results) {
		if (err) return callback(err);
...
ElasticSearchDB.prototype.userGet = function(user, appId, callback) {
	if (user.id) {
		...
		callback(new Error('User.get: no id or username supplied'));
	};
};

ElasticSearchDB.prototype.userGetAll = function(appId, offset, limit, callback) {
	this.connection.search({
		index: this.config.index,
		type: 'user',
		body: {
...
ElasticSearchDB.prototype.userCreate = function(props, appId, callback) {
	...
	this.userGet({username: props.username}, appId, function(err, result) {
		if (err && err.status == 404) {
			self.connection.create({
				index: self.config.index,
				type: 'user',
...
ElasticSearchDB.prototype.userUpdate = function(username, appId, patch, callback) {
	var self = this;
	var modifiedUser = null;

	async.waterfall([
		function(callback1) {
			self.userGet({username: username}, appId, callback1);
		},
...
ElasticSearchDB.prototype.userDelete = function(username, appId, callback) {
	var self = this;
	var user = null;
	...
	async.series([
		...
		function(callback1) {
			self.userGet({username: username}, appId, function(err, result) {
				if (err) return callback1(err);
...
ElasticSearchDB.prototype.userGetMetadata = function(userId, callback) {
	var self = this;
	...
	this.connection.search({
		...
		if (!result.hits.hits[0]) {
			...
			return callback(null, result.hits.hits[0]._source);
		}
	});
};

ElasticSearchDB.prototype.userUpdateMetadata = function(userId, patches, callback) {
	var self = this;
	var modifiedObject = null;

	async.waterfall([
		function(callback1) {
			self.userGetMetadata(userId, callback1);
		},
		function(object, callback1) {
			modifiedObject = Delta.processObject(patches, object);
