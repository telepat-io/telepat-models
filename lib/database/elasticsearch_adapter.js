			...
			if (typeof options.sort[sortFieldName] == 'string') {
				...
				Object.keys(options.sort).forEach(function(field) {
					...
					if (!options.sort[field].type) {
						...
						sortObjectField._geo_distance[field] = {lat: options.sort.poi.lat || 0.0, lon: options.sort.poi.long || 0.0};
						reqBody.sort._geo_distance.order = options.sort[field].order;
						reqBody.sort._geo_distance.unmapped_type = "long";
					}
