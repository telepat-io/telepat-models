...
Subscription.getSubscribedDevices = function(context, channel, filters, callback) {
	...
	if (channel.user)
		key += ':users:'+channel.user;
	if (channel.parent)
		key += ':'+channel.parent.model+':'+channel.parent.id;

	if (filters)
		key += (new Buffer(JSON.stringify(utils.parseQueryObject(filters)))).toString('base64');
...
Subscription.add = function(appId, deviceId, channel, extraFilters, callback) {
	var key = Subscription.getSubscriptionKey(appId, deviceId, channel, extraFilters);
...
Subscription.getSubscriptionKey = function(appId, deviceId, channel, extraFilters) {
...
Subscription.remove = function(context, deviceId, channel, filters, callback){
	...
	Application.stateBucket.getAndLock(key, function(err, results) {
		if (err && err.code == cb.errors.keyNotFound) {
			...
			error.code = cb.errors.keyNotFound;
