	...
	async.waterfall([
		...
		function(results, callback1) {
			...
			if (results.subscriptions.indexOf(channel.get()) === -1) {
				...
				Subscription.updateDevice(appId, results, null, callback1);
			}
...
Subscription.removeAllSubscriptionsFromDevice = function(deviceId, callback) {
	...
	async.waterfall([
		...
		function(result, callback1) {
			...
			deviceSubscriptions.forEach(function(subscription) {
				transaction.srem([subscription, deviceId]);
			});
...
Subscription.getAllDevices = function(appId, callback) {
	Application.redisClient.scan([0, 'MATCH', 'blg:'+appId+':devices:[^udid]*', 'COUNT', 100000], function(err, results) {
		if (err) return callback(err);

		Application.redisClient.mget(results[1], function(err, results) {
