	...
	async.series([
		...
		function(callback1) {
			...
			if (device.volatile) {
				...
				if (!device.persistent.type)
					return callback1(new TelepatError(TelepatError.errors.DeviceInvalid, [deviceId, 'persistent type is missing']));
...
Subscription.removeAllSubscriptionsFromDevice = function(appId, deviceId, callback) {
	...
	async.series([
		...
		function(callback1) {
			...
			device.subscriptions.forEach(function(subscription) {
				var transportType = '';
				var token = '';
				...
				transaction.srem([subscription, transportType+'|'+deviceId+'|'+token+'|'+appId]);
			});

			transaction.exec(callback1);
		}
	], function(err) {
		callback(err, device.subscriptions);
	});
};
