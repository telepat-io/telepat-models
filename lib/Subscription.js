...
Subscription.addDevice = function(device, callback) {
	...
	Application.stateBucket.insert(key, JSON.stringify(device), callback);
...
Subscription.updateDevice = function(id, callback) {
	var key = 'blg:devices:'+id;

	async.waterfall([
		function(callback1) {
			Subscription.getDevice(id, callback1);
		},
		function(result, callback1) {
			...
			Application.bucket.replace(key, device, callback1);
		}
	], callback);
}
