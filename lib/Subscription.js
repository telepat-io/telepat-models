...
Subscription.add = function(appId, deviceId, channel, callback) {
	...
	async.series([
		...
		function(callback1) {
			...
			Application.redisClient.sadd([channel.get(), transportType+'|'+deviceId+'|'+appId], callback1);
		},
		function(callback1) {
			device.subscriptions = results.subscriptions || [];
...
Subscription.remove = function(appId, deviceId, channel, callback){
	...
	async.series([
		...
		function(callback1) {
			...
			Application.redisClient.srem([channel, transportType+'|'+deviceId+'|'+appId], function(err, result) {
				if (err) return callback1(err);
