...
Subscription.getObjectCount = function(appId, channel, extraFilters, callback) {
		...
		query: {
			filtered: {
				query: {
					bool: {
						must: [
							{term: {'doc.type': channel.model}},
							{term: {'doc.context_id': channel.context}}
						]
					}
				}
			}
	...
	if (channel.user) {
		userQuery['doc.user_id'] = channel.user;
		elasticSearchQuery.query.filtered.query.bool.must.push({term: userQuery});
	}

	if(channel.parent) {
		parentQuery['doc.'+channel.parent.model+'_id'] = channel.parent.id;
		elasticSearchQuery.query.filtered.query.bool.must.push({term: parentQuery});
	}

	elasticSearchQuery.query.filtered.filter = utils.parseQueryObject(filters);
...
Subscription.updateDevice = function(device, callback) {
	...
	async.waterfall([
		function GetDevice(callback) {
			Subscription.getDevice(device.id, callback);
		},
