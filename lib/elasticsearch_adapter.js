...
ElasticSearchDB.prototype.applicationUpdate = function(id, patches, callback) {
	var self = this;
	var modifiedApp = undefined;

	async.waterfall([
		function(callback1) {
			self.applicationGet(id, callback1);
		},
		function(app, callback1) {
			for (var i in patches) {
				...
				if (patches.hasOwnProperty(i) && ['id', 'type', 'created', 'modified', 'application_id', 'context_id'].indexOf(objectField) == -1) {
					switch (patches[i].op) {
						case 'replace': {
							app[objectField] = patches[i].value;
			...
			app.modified = Math.floor((new Date()).getTime()/1000);
			modifiedApp = app;
...
ElasticSearchDB.prototype.applicationDelete = function(id, callback) {
	...
	async.waterfall([
		function(callback1) {
			self.contextGetAll(id, callback1);
		},
		function(callback1) {
...
ElasticSearchDB.prototype.applicationGetSchema = function(appId, callback) {
	this.connection.get({
		...
		callback(null, results._source.schema);
	});
};
...
ElasticSearchDB.prototype.adminUpdate = function(email, patches, callback) {
	var self = this;
		...
		function(admin, callback1) {
			for (var i in patches) {
				...
				if (patches.hasOwnProperty(i) && ['id', 'type', 'created', 'modified', 'application_id', 'context_id'].indexOf(objectField) == -1) {
					switch (patches[i].op) {
						case 'replace': {
							admin[objectField] = patches[i].value;
			...
			admin.modified = Math.floor((new Date()).getTime()/1000);
			updatedAdmin = admin;
