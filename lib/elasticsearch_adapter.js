...
ElasticSearchDB.prototype.adminGet = function(admin, callback) {
	if (admin.id) {
		this.connection.get({
			...
			if (!result.found) {
				var error = new Error('Admin with id "'+admin.id+'" not found');
				error.status = 404;
		...
		this.connection.search({
			...
			if (result.hits.total == 0) {
				var error = new Error('Admin with email address "'+admin.email+'" not found');
				error.status = 404;
...
ElasticSearchDB.prototype.adminUpdate = function(patches, callback) {
	...
	async.waterfall([
		...
		if (err && err.status == 404) {
			var error = new Error('Admin with id "'+adminId+'" does not exist');
			error.status = 404;
