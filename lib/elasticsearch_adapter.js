...
ElasticSearchDB.prototype.adminCreate = function(email, props, callback) {
	...
	async.series([
		function(callback1) {
			self.adminGet(email, function(err, admin) {
				if (err && err.status == 404)
					return callback1();
				else if (err)
					return callback1(err);
				else {
					var error = new Error('Admin with that email address already exists');
					error.status = 409;
					callback1(error);
				}
			});
...
ElasticSearchDB.prototype.adminUpdate = function(email, props, callback) {
	...
	async.waterfall([
		...
		if (err && err.status == 404) {
			var error = new Error('Admin with email address "'+email+'" does not exist');
			error.status = 404;
...
ElasticSearchDB.prototype.adminDelete = function(email, callback) {
	var self = this;
	var admin = null;
	...
	async.series([
		function(callback1) {
			self.adminGet(email, function(err, response) {
				if (err) return callback1(err);
