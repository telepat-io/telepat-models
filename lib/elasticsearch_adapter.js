...
ElasticSearchDB.prototype.modelGet = function(name, appId, context_id, id, callback) {
	this.connection.get({
		...
		if (response.found == false) {
			callback(new TelepatError(TelepatError.errors.ObjectNotFound, [name, id]));
		}
			...
			if (parent) {
				self.modelGet(parent.model, appId, props.context_id, parent.id, function(err, result) {
					if (err && err.status == 404) {
						...
						return callback1(err);
					} else if (result[Application.loadedAppModels[appId][parent.model].hasSome_property] &&
						result[Application.loadedAppModels[appId][parent.model].hasSome_property].length <= parentRelationKey) {

						callback1(new TelepatError(TelepatError.errors.InvalidObjectRelationKey,
							[
								parentRelationKey,
								result[Application.loadedAppModels[appId][parent.model].hasSome_property].length-1
							]));
					} else
						callback1();
				});
