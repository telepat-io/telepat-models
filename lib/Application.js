...
Application.loadAppModels = function(appId, callback) {
	...
	Application.bucket.get(key, function(err, result) {
		...
		Application.loadedAppModels[appId] = models.models;
		callback();
	});
}
...
Application.createSchema = function(appId, newSchema, callback) {
	var key = 'blg:'+Application._model.namespace+':'+appId+':schemas';

	var schema = {};
	//schema.models[name] = props;

	for(var s in newSchema) {
		schema[s] = newSchema[s];
	}

	Application.bucket.insert(key, schema, callback);
}
...
Application.updateSchema = function(name, appId, props, callback) {
	var key = 'blg:'+Application._model.namespace+':'+appId+':schemas';
	...
	async.waterfall([
		...
		function(result, callback1) {
			...
			Application.bucket.replace(key, models, callback1);
		}
	], callback);
}

Application.deleteSchema = function(name, appId, callback) {
	var key = 'blg:'+Application._model.namespace+':'+appId+':schemas';
