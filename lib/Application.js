...
class TelepatApplication extends BaseModel {
	...
	static getIds() {
		...
		for(let r in TelepatApplication.apps) {
			ids.push(TelepatApplication.apps[r].id);
		}
		return ids;
	}
	...
	static isValidModel(appId, modelName) {
		if (builtinModels.indexOf(modelName) !== -1) {
			return new TelepatError(TelepatError.errors.InvalidFieldValue, modelName);
		}

		if(!TelepatApplication.apps || !TelepatApplication.apps[appId]) {
			return new TelepatError(TelepatError.errors.ApplicationNotFound, appId);
		}

		if(!TelepatApplication.apps[appId].schema) {
			return new TelepatError(TelepatError.errors.ApplicationHasNoSchema);
		}

		if(!TelepatApplication.apps[appId].schema[modelName]) {
			return new TelepatError(TelepatError.errors.ApplicationSchemaModelNotFound, appId, modelName);
		}
	...
	modelSchema(modelName) {
		...
		schemaObj.belongsTo = (parentName) => {
		...
		schemaObj.hasParent =  (modelParent) => {
			return TelepatApplication.hasParent(this.id, modelName, modelParent);
		};

		schemaObj.hasSome = (modelParent) => {
			if(schemaObj[modelParent] && schemaObj[modelParent].hasSome && (schemaObj[modelParent].hasSome.indexOf(modelName) !== -1)) {
				return schemaObj[modelParent].hasSome_property;
			}
			return false;
		};

		schemaObj.hasMany = (modelParent) => {
			return (schemaObj[modelName].hasMany && schemaObj[modelName].hasMany.indexOf(modelParent) !== -1);
		}; 

		schemaObj.isValidModel = () => {
			return TelepatApplication.isValiidModel(this.id, modelName);
		}
	...
	deleteModel(modelName, callback) {
		let appId = this.id;
		let validation = TelepatApplication.isValidModel(appId, modelName);
