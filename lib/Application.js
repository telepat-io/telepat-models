...
class TelepatApplication extends BaseModel {
	...
	static new(props, callback) {
		props.id = guid.v4();
		TelepatApplication.apps[props.id] = new TelepatApplication(props);
		Services.datasource.dataStorage.createObjects([TelepatApplication.apps[props.id].properties], (errs) => {
			if (errs) {
		...
		async.waterfall([
			callback1 => {
				Services.datasource.dataStorage.updateObjects(patches, (errs, apps) => {
					...
					TelepatApplication.apps[id] = new TelepatApplication(apps[id]);
	...
	static belongsTo(application_id, modelName) {
		if(TelepatApplication.check(application_id, modelName)) {
			return TelepatApplication.check(application_id, modelName);
		}
		let belongs = [];
		let appModels = TelepatApplication.apps[application_id].schema; 
		if (!appModels[modelName] || !appModels[modelName].belongsTo) {
			return new TelepatError(TelepatError.errors.ApplicationSchemaModelNotFound);
		}
	...
	model(modelName) {	
		return {
			...
			check: TelepatApplication.check(this.id, modelName)
		};
	}
