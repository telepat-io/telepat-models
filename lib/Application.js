...
class TelepatApplication extends BaseModel {
	...
	static isValidModel(id, modelName) {
		if (builtinModels.indexOf(modelName) !== -1) {
	...
	static belongsTo(application_id, modelName) {
		...
		let validation = TelepatApplication.isValidModel(application_id, modelName);
	...
	modelSchema(modelName) {
		let schema = this.properties.schema; 
		let schemaObj = schema;
		schemaObj.belongsTo = (parentName) => {
			if (schema[modelName].belongsTo) {
				return !!schema[modelName].belongsTo.find(parent => parent.parentModel === parentName);
			}
			return false;
		};
		...
		schemaObj.hasSome = (modelParent) => {
			return !!(schema[modelName].hasSome.indexOf(modelParent) !== -1);
		};

		schemaObj.hasMany = (modelParent) => {
			return !!(schema[modelName].hasMany.indexOf(modelParent) !== -1);
		}; 
