 ...
 * @type {Bucket}
 */
Application.bucket = null;

/**
 *
 * @type {Client}
 */
Application.elasticClient = null;
...
Application.loadAppModels = function(appId, callback) {
	var key = 'blg:'+Application._model.namespace+':'+appId+':schema';
...
Application.setRedisClient = function(redis) {
	Application.redisClient = redis;
};

Application.setElasticClient = function(client) {
	Application.elasticClient = client;
}

Application.setCB = function(cb) {
	global.cb = cb;
}
...
Application.getAll = function(callback) {
	var query = cb.ViewQuery.from('dev_models', 'by_type').custom({ inclusive_end: true, stale: false, key: '"application"'});
...
Application.delete = function (id, callback) {
	async.waterfall([
		...
		function(callback1) {
			var key = 'blg:'+Application._model.namespace+':'+id+':schema';
...
Application.hasContext = function(appId, contextId, callback) {
	var key = 'blg:'+Application._model.namespace+':'+appId+':contexts';

	Application.bucket.get(key, function(err, result) {
		if (err) return callback(err);

		var idx = result.value.search(contextId+',');
