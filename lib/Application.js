...
Application.updateSchema = function(name, appId, props, callback) {
	...
	Application.bucket.get(key, function (err, res) {
		...
		if (err)
			...
			currentSchema = res.value

		currentSchema[name] = props;

		Application.bucket.upsert(key, currentSchema, null, callback);
	});
}
...
Application.deleteSchema = function(name, appId, callback) {
	...
	Application.bucket.get(key, function (err, res) {
		...
		Application.bucket.replace(key, currentSchema, callback);
	});
}
