...
function Model(name, appId, context_id, _id, callback) {
	if (!Application.loadedAppModels[appId]) {
		throw new TelepatError(TelepatError.errors.ApplicationNotFound, [appId]);
		return;
	}
	if (!Application.loadedAppModels[appId][name]) {
		throw new TelepatError(TelepatError.errors.ApplicationSchemaModelNotFound, [appId, name]);
		return;
	}
...
Model.search = function(channel, callback) {
	Application.datasource.dataStorage.modelSearch(channel, callback);
};

Model.countByQuery = function(channel, callback) {
