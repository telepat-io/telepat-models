...
function Model(name, appId, _id, callback) {
	...
	Application.bucket.query(query, (function(err, res) {
		...
		for(var prop in Application.loadedAppModels[appId][name].properties) {
			if (Application.loadedAppModels[appId][name].properties.hasOwnProperty(prop)) {
...
Model.delete = function(modelName, appId, id, callback) {
	var item = null;
    var key = null;
	//user_id
	//parent
...
Model.create = function(modelName, appId, props, callback1) {
	...
	async.waterfall([
		...
		function(count, callback) {
			...
			for(var prop in Application.loadedAppModels[appId][modelName].properties) {
				if (Application.loadedAppModels[appId][modelName].properties.hasOwnProperty(prop)) {
...
Model.update = function(modelName, appId, id, patch, callback) {
	...
	async.waterfall([
		...
		function(item, get_cb) {
			...
			for (var i in patch) {
				if (patch.hasOwnProperty(i) && Application.loadedAppModels[appId][modelName].properties[patch[i].path]) {
					switch (patch[i].op) {
						case 'replace': {
