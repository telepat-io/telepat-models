...
Model.delete = function(modelName, context, id, callback) {
	...
	async.waterfall([
		...
		for (var r in Model._spec[modelName].belongsTo) {
			if (item[Model._spec[modelName].belongsTo[r]+'_id'])
 ...
 * @param cb
 */
Model.update = function(modelName, context, id, props, user_id, parent, callback) {
	...
	acceptedProps.modified = Math.floor((new Date()).getTime()/1000);
