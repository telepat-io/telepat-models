...
class Model extends BaseModel {
	...
	constructor(props) {
		props.id = guid.v4();
		let validationCheck = Application.check(props.application_id, props.type);
	...
	static getParent(object) {
		...
		console.log(application_id, modelName);
		if(Application.check(application_id, modelName)) {
			return Application.check(application_id, modelName);
		}
		let appModels = Application.apps[application_id].schema;
		console.log(Application.apps[application_id]);
		...
		async.series([
			(callback1) => {
				async.forEachOf(deltas, (d, i, c) => {
					...
					let validationCheck = Application.check(object.application_id, object.type);	
					if(validationCheck instanceof TelepatError) {
						return c();
					}
					...
					Services.datasource.dataStorage.getObjects(Object.keys(objParentInfo), (errs, results) => {
						...
						if (results && results.length >= 1) {
							...
							deltas.forEach((delta, i) => {
								...
								let check = Application.check(obj.application_id, obj.type);
			...
			(callback1) => {
				console.log("cureated deltas = ", curatedDeltas);
				...
				() => {
					console.log('app models ', appModelsObjects);
