	...
	async.waterfall([
		...
		for (var r in Model._spec[modelName].belongsTo) {
			if (item[Model._spec[modelName].belongsTo[r].parentModel+'_id'])
				var parentModel = Model._spec[modelName].belongsTo[r].parentModel.charAt(0).toUpperCase() +
					Model._spec[modelName].belongsTo[r].parentModel.slice(1);

				parent = {model: parentModel, id: item[Model._spec[modelName].belongsTo[r].parentModel+'_id']};
		}
