...
class Model extends BaseModel {
	...
	constructor(props) {
		...
		props.parent_info = Model.getParentInfo(props) || false;
		...
		async.series([
			(callback1) => {
				async.forEachOf(deltas, (d, i, c) => {
					...
					if (Application.isValidModel(object.application_id, object.type) instanceof TelepatError) {
						return c(); 
					}
					if(object.parent_info && object.parent_info.id) {
						objParentInfo[object.parent_info.id] = object.parent_info;
					}
	...
	static getParentInfo(object) {
		...
		let validationCheck = Application.isValidModel(application_id, modelName);
