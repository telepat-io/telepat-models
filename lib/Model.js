...
Model.load = function(app) {
	...
	Model._spec.properties.context_id = {type: "integer"};
};
...
Model.getAll = function(modelName, context, callback) {
	var query = cb.ViewQuery.from('dev_models', 'by_type_context').custom({
		"startkey": "["+context+","+Model._spec[modelName].type+"]",
		"endkey": "["+context+","+Model._spec[modelName].type+"]"
	});
...
Model.delete = function(modelName, context, id, user_id, parent, callback) {
	...
	Application.bucket.remove(key, function(err, results) {
		...
		Application.bucket.getAndLock(lookupKey, function(err1, results1) {
			...
			Application.bucket.replace(lookupKey, results.splice(idx, 1).toString()+',', function(err2, results2){
...
Model.create = function(modelName, context, props, user_id, parent, callback) {
	Model.count(modelName, function(err, results) {
		if (err) {
			callback(err, null); return;
		}

		var idx = results.value+1;
		...
		Application.bucket.insert(directKey, acceptedProps, function(err1, results1) {
			...
			Application.bucket.append(lookupKey, idx+',', callback);
		});
...
Model.update = function(modelName, context, props, user_id, parent, callback) {
	...
	if (props.author) {
		...
		Application.bucket.get(lookupKey, function(err, results){
			...
			Application.bucket.replace(lookupKey, results.splice(idx, 1).toString()+',', function(err, results) {});
