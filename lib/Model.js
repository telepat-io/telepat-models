...
class Model extends BaseModel {
	...
	constructor(props) {
		let validationCheck = Application.getError(props.application_id, props.type);
		...
		async.series([
			(callback1) => {
				async.forEachOf(deltas, (d, i, c) => {
					...
					if (!Application.isValid(object.application_id, object.type)) {
						return c();
					}
					...
					Services.datasource.dataStorage.getObjects(Object.keys(objParentInfo), (errs, results) => {
						...
						if (results && results.length >= 1) {
							...
							deltas.forEach((delta, i) => {
								...
								if(!Application.isValid(obj.application_id, obj.type)) {
									return; 
								}

								let appModels = Application.apps[obj.application_id].schema;
								let parent = Model.getParent(obj);
	...
	static getParent(object) {
		...
		if(!Application.isValid(application_id, modelName)) {
			return Application.getError(application_id, modelName);
		}
