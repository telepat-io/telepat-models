...
class Model extends BaseModel {
	...
	constructor(props) {
		...
		let validationCheck = Model.belonsTo(object);

		if(validationCheck instanceof TelepatError) {
			return validationCheck;
		}

		const proxiedParent = super(object, ['context_id', 'application_id', 'user_id', 'parent_info.id']);
	...
	static belonsTo(object) {
		...
		if(!Application.apps[object.application_id]) {
			return new TelepatError(TelepatError.errors.ApplicationIdMissing);
		}

		if(!Application.apps[object.application_id].schema) {
			...
			(callback1) => {
				async.forEachOf(deltas, (d, i, c) => {
					...
					if(validationCheck.id) {
						objParentInfo[validationCheck.id] = validationCheck;
					}
