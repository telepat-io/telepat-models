...
Model.create = function(deltas, callback, returnedObjectsCb) {
	...
	async.series([
		function(callback1) {
			...
			returnedObjectsCb(curatedDeltas.map(function(d) {
				delete d.object._metadata;
...
Model.update = function(patches, callback) {
	...
	async.eachSeries(patches, function(p, c) {
		...
		if (appModelsPatches.length) {
			Application.datasource.dataStorage.updateObjects(appModelsPatches, function(errs, modified) {
				if (errs && errs.length >= 1) {}
				errs.forEach(function(error) {
					if (error.status == 404)
