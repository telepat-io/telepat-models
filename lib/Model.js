...
class Model extends BaseModel {
	constructor(props) {
		let validationCheck = Application.isValidModel(props.application_id, props.type);
					...
					Services.datasource.dataStorage.getObjects(Object.keys(objParentInfo), (errs, results) => {
						...
						if (results && results.length >= 1) {
							...
							deltas.forEach((delta) => {
								...
								if(Application.isValidModel(obj.application_id, obj.type) instanceof TelepatError) {
									return; 
								}
					...
					if (result) {
						...
						async.waterfall([
							...
							(redisResult, callback2) => {
								if(redisResult !== null) {
									Services.countObjects({modelName: channel.props.model, filters: filters, aggregation: aggregation}, callback2);
								} else {
	...
	static getParentInfo(object) {
		...
		let validationCheck = Application.isValidModel(appId, modelName);
		...
		for(let r in appModels[modelName].belongsTo) {
			if(object[appModels[modelName].belongsTo[r].parentModel + '_id']) {
				...
				console.log(appModels[modelName].belongsTo[r]);
				relationType = appModels[modelName].belongsTo[r].relationType;
			}
		}
		let objParentInfo;
		console.log('getParentInfo 3', parent);
		if(parent) {
