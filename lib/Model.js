...
Model.delete = function(modelName, context, appId, id, onlyChild, callback) {
	...
	async.waterfall([
		function(get_cb) {
			new Model(modelName, context, appId, id, function(err, result) {
		...
		function(callback1) {
			...
			async.parallel([
				function(callback2) {
					var childName = Application.loadedAppModels[appId][modelName].hasMany;
					if (childName) {
				...
				function(callback2) {
					var childName = Application.loadedAppModels[appId][modelName].hasSome;
					if (childName) {
						var countKey = 'blg:'+item.context_id+':';
...
Model.create = function(modelName, appId, props, callback1) {
	...
	async.waterfall([
		function(callback) {
			if (parent) {
				Model(parent.model, props.context_id, appId, parent.id, function(err, result) {
					if (err && err.code == cb.errors.keyNotFound) {
						var error = new Error('Unable to create'.red+': parent "'+parent.model+'" with id '+parent.id+' does not exist');
						callback(error);
					} else if (err)
						callback(err);
					else if (result[Application.loadedAppModels[appId][parent.model].hasSome_property].length <= parentRelationKey) {
